...
let fOfficers = humanOfficers.filter { femaleOfficers.contains($0) }
let percentage = (Double(fOfficers.count) / Double(humanOfficers.count)) * 100
print("Percentage:")
print(percentage)


// Q2: How many first officers are human?
let firstOfficersHuman = firstOfficers.keys.filter { humanOfficers.contains($0) }.count
print(firstOfficersHuman)


// Q3: Get a list of females sorted by name
let sorted1 = femaleOfficers.sorted()
print(sorted1)
...
let captainOfficers = captains.map{ Officer(name: $0, ship: $1)}
print( captainOfficers.map{$0.name})



// Q5: Alphabetize the previous list by the captain's name
let sortedcaptainOfficers = captainOfficers.sorted()
print( sortedcaptainOfficers.map{$0.name})
...
let countNames = vulcanFirstOfficers.reduce(0) { $1.name.count < 5 ? $0 + 1 : $0 }
print(countNames)
